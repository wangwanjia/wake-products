<template>
  <div class="tabs-get-open">
    ...
    <div class="bg-white p-4 rounded-lg shadow-md">
      <div ref="chartHongKongRef" class="w-full! h-[400px]"></div>
    </div>
  </div>
</template>

<script setup>
defineOptions({ name: "GetOpenCodeHongKong" });
...
// 统计图表 按照年份展示
const chartHongKongRef = ref(null);
const chartHongKong = ref(null);
...
watch(
  ...
  () => {
    ...
    if (chartHongKong.value) {
      chartHongKong.value.setOption(chartOptions.value);
    }
  },
  { deep: true }
);
...
onMounted(() => {
  chartHongKong.value = echarts.init(chartHongKongRef.value);
  chartHongKong.value.setOption(chartOptions.value);
  // 监听窗口大小变化，自动调整图表大小
  window.addEventListener("resize", () => {
    chartHongKong.value.resize();
  });
});
watch(
  () => props.isActive,
  (newVal) => {
    if (newVal) {
      nextTick(() => {
        console.log("chartHongKong.value2222", chartHongKong.value);
