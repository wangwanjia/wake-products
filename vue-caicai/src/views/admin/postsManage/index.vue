  ...
  <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
    <el-tab-pane name="index">
      ...
      <div class="mb-6 bg-white p-4 rounded-lg shadow-md">
        ...
        <!-- 内容 -->
        <h3 class="text-lg font-semibold mb-4 text-gray-800">帖子管理</h3>
        ...
        >
          ...
          <div class="flex">
            <el-button
              ...
              >批量删除</el-button
            >
            <el-button type="primary" class="ml-2" @click="addPosts"
              >添加帖子</el-button
            >
          </div>
        </div>
...
const currentDeletePosts = ref(null);
...
const handleDelete = (posts) => {
  currentDeletePosts.value = posts;
  deleteDialogVisible.value = true;
};

// 确认删除
const confirmDelete = () => {
  if (currentDeletePosts.value) {
    // 这里可以添加实际的删除API调用
    // 模拟删除操作
    const index = postsData.findIndex(
      (item) => item.id === currentDeletePosts.value.id
    );
    if (index !== -1) {
      postsData.splice(index, 1);
      console.log("删除帖子:", currentDeletePosts.value);
    }
    deleteDialogVisible.value = false;
    currentDeletePosts.value = null;
  }
};
...
const confirmBatchDelete = () => {
  if (multipleSelection.value.length > 0) {
    ...
    postsData.length = 0;
    postsData.push(...newPostsData);
    console.log("批量删除帖子数:", multipleSelection.value.length);
...
const handleEdit = (index, row) => {
  // activeName.value = 'child'
  router.push({
    path: "/admin/postsManage/postsEdit",
    query: {
      id: row.id,
    },
  });
...
const addPosts = () => {
  router.push({
    path: "/admin/postsManage/postsAdd",
  });
};
